...
void Model::Draw(Shader& shader, Camera& camera, glm::vec3 translation, glm::quat rotation, glm::vec3 scale)
{
	...
	{
		// Apply translation, rotation, and scale to the matrix
		glm::mat4 modelMatrix = matricesMeshes[i];
		modelMatrix = glm::translate(modelMatrix, translation);
		modelMatrix *= glm::mat4_cast(rotation); // mat4_cast converts a quaternion to matrix
...
std::vector<Texture> Model::getTextures()
{
	...
	for (unsigned int i = 0; i < JSON["images"].size(); i++)
	{
		...
		if (!skip)
		{
			...
			else if (texPath.find("metallicRoughness") != std::string::npos || texPath.find("specular"))
			{
